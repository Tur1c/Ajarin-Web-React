...
function getCategoryData(
  ...
  let discData = studentDisc
    .filter(
      (x) =>
        x.discussion.category.category_id != "99" &&
        dayjs(x.joined_date).month() === currMonth
    )
...
function countSubject(
  ...
  let total = studentDisc.filter(
    (x) =>
      dayjs(x.joined_date).month() === currMonth &&
      x.discussion.category.category_name === category
  ).length;
  total =
    total +
    studentCourse.filter(
      (x) =>
        dayjs(x.joined_date).month() === currMonth &&
        x.course.category.category_name === category
    ).length;
  // console.log(total, category);
  return total;
}

function countSubjectsTotal(
  studentDisc: StudentDisc[],
  studentCourse: StudentCourse[],
  label: string[],
  currMonth: any
...
export function createCurrMonthPie(
  acc: AccountOutput,
  currMonth: any
): PieChartData {
  console.log(currMonth, "bulan berapa");
  const label = getCategoryData(
    acc.studentdisc_list,
    acc.studentcourse_list,
    currMonth
  );
  // console.log(label);
  const subjectCount = countSubjectsTotal(
    acc.studentdisc_list,
    acc.studentcourse_list,
    label,
    currMonth
...
function getHoursSpent(
  ...
  let discData = studentDisc.filter(
    (x) =>
      x.discussion.category.category_id != "99" &&
      x.status.localeCompare("Completed") === 0 &&
      dayjs(x.discussion.disc_date).month() === currMonth
  );
  let result: number[] = [];
  for (let idx = 0; idx < total; idx++) {
    ...
    // disc
    let usedDiscData = discData.filter(
      (x) => getISOWeek(x.discussion.disc_date) === weeks[idx]
    );
    if (usedDiscData) {
      usedDiscData.forEach((x) => {
        console.log(
          dayjs(x.discussion.disc_starttime, "HH:mm:ss").format("HH:mm:ss"),
          x.discussion.disc_endtime
        );
        var z: number = dayjs(x.discussion.disc_endtime, "HH:mm:ss").diff(
          dayjs(x.discussion.disc_starttime, "HH:mm:ss"),
          "minute"
        );
        if (z < 0) z = 1440 + z;
        result[idx] += z;
    ...
    // course
    const data: string[] = studentCourse
      .filter((x) => x.completed_chap !== null)
      .map((x) => x.completed_chap);
    data.forEach((x) => {
      var totalData = x
        .split("|")
        ...
        .filter(
          (x) =>
            dayjs(x).month() === currMonth &&
            getISOWeek(new Date(x)) === weeks[idx]
        ).length;
...
export function createCurrMonthBar(
  acc: AccountOutput,
  currMonth: any
): BarChartData {
  var firstDayofMonth = new Date(
    dayjs().month(currMonth).set("date", 1).format("YYYY-MM-DD")
  );
  ...
  var totalWeeks = Math.ceil(
    (firstDayofMonth.getDay() + lastDayofMonth.getDate()) / 7
  );
  var firstWeek = getISOWeek(firstDayofMonth);

  console.log(
    acc.studentcourse_list
      .filter((x) => x.status.localeCompare("Completed") === 0)
      .map((x) =>
        x.completed_chap.substring(x.completed_chap.lastIndexOf(";") + 1)
      )
  );
...
export function getTotalCourse(acc: AccountOutput, currMonth: any) {
  var totalCourse = acc.studentcourse_list.filter(
    (x) =>
      x.status.localeCompare("Completed") === 0 &&
      dayjs(
        x.completed_chap.substring(x.completed_chap.lastIndexOf(";") + 1)
